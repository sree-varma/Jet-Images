...
def getImage(filename):
    ...
    filenameQ = tf.train.string_input_producer([filename],num_epochs=None,shuffle = True)
    ...
    image=tf.reshape(image,[height*width*channels])#tf.image.grayscale_to_rgb()
    a = image.shape
    print a
    ...
    #c,d = label.shape
    #label=tf.reshape(label,[nClass,1])
    return label, image
...
label, image = getImage("/usr/ml_jet_images/train-00000-of-00001")


# and similarly for the validation data
vlabel, vimage = getImage("/usr/ml_jet_images/validation-00000-of-00001")
...
imageBatch, labelBatch = tf.train.shuffle_batch(
    ...
    capacity=90000,
    min_after_dequeue=45000)
print imageBatch.shape
# and similarly for the validation data 
vimageBatch, vlabelBatch = tf.train.shuffle_batch(
    [vimage, vlabel], batch_size=128,
    capacity=10000,
...
if simpleModel:
  ...
  W = tf.Variable(tf.zeros([width*height*channels, nClass]))
  ...
  # 1 input channel
  W_conv1 = weight_variable([8, 8, 3, nFeatures1])
  ...
  W_conv2 = weight_variable([4, 4, nFeatures1, nFeatures2])
  ...
  W_conv3 = weight_variable([4, 4, nFeatures2, nFeatures3])
  ...
  W_fc1 = weight_variable([(width/8) * (height/8) * nFeatures3, nNeuronsfc])
  b_fc1 = bias_variable([nNeuronsfc])
  
  # flatten output from previous layer
  h_pool3_flat = tf.reshape(h_pool3, [-1, (width/8) * (height/8) * nFeatures3])
